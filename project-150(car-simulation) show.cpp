...
HANDLE console = GetStdHandle(STD_OUTPUT_HANDLE);//handle object or character in the handle
COORD CursorPosition;//for set the current cursor position
...
void gotoxy(int x, int y) {//move the cursor specific position
    CursorPosition.X = x;
    CursorPosition.Y = y;
    SetConsoleCursorPosition(console, CursorPosition);//api that move the cursor
}

void setcursor(bool visible, DWORD size) {//visibility and size of the console cursor
    if (size == 0)
        size = 50;
...
void resetEnemy(int ind) {//reset and genenemy
    eraseEnemy(ind);
    enemyY[ind] = 1;
    genEnemy(ind);
}
...
void play() {
    ...
    drawBorder();
    updateScore();
    genEnemy(0);//generate 1st enemy and
    genEnemy(1);//denerate 2nd enemy
